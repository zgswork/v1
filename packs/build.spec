...
a = Analysis(
    ...
    datas=[                                  #这是非常重要的列表，它指定了非 Python 文件（静态资源、模板、配置文件等）如何被复制到打包后的目录中。每条是一个元组 (源路径, 目标目录)：
        #('index.html', '.'),                 # 将根目录的 index.html 放在 exe 同级
        ('pages', 'pages'),                  # 整个文件夹
        ('static', 'static'), 
        ('pysub','pysub'),
        #('app.ico', '.'),                    #将根目录的 app.ico 复制到 exe 同级目录,以供托盘图标使用
    ],
...
pyz = PYZ(a.pure)                            #a.pure 是 Analysis 收集到的所有纯 Python 模块（即 .py 文件）列表。PYZ 会将这些模块打包成一个压缩的 .pyz 文件，嵌入到最终的 exe 中，从而减少外部文件数量并提高加载效率。
...
exe = EXE(                                   #前面打包好的 pyz 归档
    ...
    a.datas,                                 # 收集到的数据文件
    [],                                      # 额外要添加的脚本（留空）
    name='LED显示屏设计工具集V1.0.2',                # 生成的 exe 文件名（不含 .exe）
    ...
    icon='static/images/app.ico',                          #可指定图标路径，如 'app.ico'
)